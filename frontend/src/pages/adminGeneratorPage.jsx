...
const AdminGeneratorPage = () => {
  ...
  const handleGenerate = async () => {
    try {
      ...
      const data = await getDataAPI('generatecode', "POST", {
      ...
      <div className='h-screen'>
        <NavegationBar currentPage={window.location.href}/>
        {
          ...
          <div className='w-8/12 max-xl:w-full h-[90%]'>
            <h1 className={`inter text-5xl mt-5 max-xl:w-11/12 max-xl:m-auto max-xl:mt-5 max-xl:mb-5`}>Generador de codigos</h1>
            <div className={`${cart.length === 0 ? "":"border-1"} w-10/12 max-xl:w-11/12 max-xl:m-auto h-full max-xl:h-4/6 mt-5 rounded-md overflow-y-auto overflow-x-hidden pb-2`}>
              {cart.length === 0 ? 
              <p className='flex h-full inter400 text-xl items-center justify-center absolute top-0 right-0 left-0 bottom-0'>Agrega productos al carrito</p>
              :
          ...
          {cart.length === 0 ? 
            ...
            <>
              ...
              <div className='w-3/12 mt-5 flex flex-col justify-start inter400 mr-10 max-xl:w-11/12 max-xl:m-auto max-xl:shadow-medium max-xl:rounded-lg max-xl:mb-4 max-xl:p-5 shadow-black'>
                <h1 className='inter text-3xl' onClick={()=>{console.log(cart)}}>Resumen</h1>
                ...
                <div className='max-xl:flex max-xl:justify-between max-sm:block'>
                  <Button onClick={handleGenerate} color="primary" variant="shadow" className='mt-2 w-full max-xl:h-12' radius='sm'>Generar</Button>
                </div>
                {
                  code && 
                  <>
                  <p className='inter400 mt-2'>Tu codigo generado es: <strong>{code}</strong></p>
                  <Button variant='ghost' className='mt-2 inter400 w-full' color='primary' onClick={() => {showNotification("Copiado al portapapeles",'success');navigator.clipboard.writeText(code)}}>Copiar</Button>
                  </>
                }
                </div>
              </div>
